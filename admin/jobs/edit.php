...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCsrf($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($errors)) {
            ...
            flashMessage('success', 'Job listing updated successfully!');
            redirect('../jobs.php');
        }
    }
}

// Pre-fill with POST data on validation error, or existing job data
$fill = ($_SERVER['REQUEST_METHOD'] === 'POST') ? $_POST : $job;
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Edit Job – <?= APP_NAME ?> Admin</title>
...
<body>
    ...
    <main class="container section">
        ...
        <div class="card form-card">
            <div class="card-body">
                <form method="POST" action="edit.php?id=<?= $id ?>" class="form">
                    <?= csrfField() ?>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" id="title" name="title" class="form-control" required
                                value="<?= htmlspecialchars($fill['title'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                        </div>
                        <div class="form-group">
                            <label for="company" class="form-label">Company <span class="required">*</span></label>
                            <input type="text" id="company" name="company" class="form-control" required
                                value="<?= htmlspecialchars($fill['company'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" id="location" name="location" class="form-control" required
                                value="<?= htmlspecialchars($fill['location'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                        </div>
                        <div class="form-group">
                            <label for="salary_range" class="form-label">Salary Range</label>
                            <input type="text" id="salary_range" name="salary_range" class="form-control"
                                value="<?= htmlspecialchars($fill['salary_range'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <select id="status" name="status" class="form-control">
                                <option value="active"   <?= ($fill['status'] ?? '') === 'active'   ? 'selected' : '' ?>>Active</option>
                                <option value="inactive" <?= ($fill['status'] ?? '') === 'inactive' ? 'selected' : '' ?>>Inactive</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="description" name="description" class="form-control" rows="8" required><?= htmlspecialchars($fill['description'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
                    </div>

                    <div class="form-group">
                        <label for="requirements" class="form-label">Requirements</label>
                        <textarea id="requirements" name="requirements" class="form-control" rows="6"><?= htmlspecialchars($fill['requirements'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
                    </div>
